...
const ProductNew = () => {
  ...
  // Clear image and reset AI-filled fields
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.frontImage) productData.frontImage = await toBase64(await compressImage(formData.frontImage));
      if (formData.rearImage) productData.rearImage = await toBase64(await compressImage(formData.rearImage));
      if (formData.otherImages?.length > 0) {
        productData.otherImages = JSON.stringify(
          await Promise.all(formData.otherImages.map(async img => toBase64(await compressImage(img))))
        );
      } else { delete productData.otherImages; }
    ...
    <div className="bg-white min-h-full">
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-40">
        <div className="container mx-auto px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div>
            <Link to="/products" className="px-4 py-2 border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 text-sm font-medium transition-all shadow-sm">
              Back
            </Link>
          </div>
      ...
      <div className="px-4 py-4 max-w-full">
        {extractionBanner && (
          <div className="mb-3 bg-blue-50 border border-blue-200 rounded px-3 py-2 flex items-center justify-between">
            ...
            <button onClick={() => setExtractionBanner(null)} className="text-blue-400 hover:text-blue-600 text-xl leading-none ml-3">&times;</button>
          </div>
        )}
        {error && (
          <div className="mb-3 bg-red-50 border border-red-200 rounded px-3 py-2 flex items-center justify-between">
            <p className="text-red-700 text-sm">{error}</p>
            <button onClick={() => setError(null)} className="text-red-400 hover:text-red-600 text-xl leading-none ml-3">&times;</button>
          </div>
        )}
        ...
        {designers.length === 0 ? (
          <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">Add at least one active designer before creating products</p>
              ...
              <div className="lg:col-span-2 bg-white rounded-lg px-5 pt-4 pb-2">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div style={{ flexShrink: 0, width: 120 }}>
                      ...
                      <div className="relative" style={{ width: 120, height: 110 }}>
                        ...
                        >
                          {formData.frontImage ? (
                            <img src={URL.createObjectURL(formData.frontImage)} alt="Preview" className="w-full h-full object-cover" />
                          ) : (
                            <>
                ...
                <div className="flex items-center gap-4 pb-5 pt-2">
                  <button type="submit" disabled={loading}
                    className="px-6 py-2.5 bg-[#1F3A2E] text-white font-bold rounded-lg hover:bg-[#243d32] transition-colors text-sm">
                    {loading ? 'Creating...' : 'Create Product'}
                  </button>
                  <Link to="/products" className="text-sm text-gray-600 hover:text-gray-900 font-medium transition-colors">
                    Cancel
                  </Link>
                </div>
              </div>
              ...
              <div className="lg:col-span-1">
                <div style={{ position: 'sticky', top: 120 }}
                  ...
                  <div className="bg-gradient-to-br from-gray-100 to-gray-200 rounded-lg flex items-center justify-center mb-3 overflow-hidden"
                    style={{ height: 260 }}>
                    {formData.frontImage
                      ? <img src={URL.createObjectURL(formData.frontImage)} alt="Preview" className="w-full h-full object-contain" />
                      : <p className="text-xs text-gray-400">No image</p>
                    }
                  </div>

                  {/* Compact details — all 12px */}
